...
static void spi_write(uint16_t w_data)
{
	...
	CS_LORA_L();
	HAL_SPI_Transmit(&hspi1, &b_data, 1, 100);
...
static void spi_burst_write(uint8_t b_addr ,uint8_t *ab_data, uint16_t w_size)
{
	...
	CS_LORA_L();
	HAL_SPI_Transmit(&hspi1, &b_tmp, 1, 100);
	HAL_SPI_Transmit(&hspi1, ab_data, w_size, 100);
	CS_LORA_H();
}
...
{
	CS_LORA_L();
	HAL_SPI_Transmit(&hspi1, &b_addr, 1, 100);
	HAL_SPI_Receive(&hspi1, ab_data, w_size, 100);
	CS_LORA_H();
}
